...
void CreateMiniDump(struct _EXCEPTION_POINTERS* exceptionInfo)
{
	...
	sprintf(tmpStr, "PsyCrossCrash.dmp");
	...
	if (pDump != NULL && hFile != INVALID_HANDLE_VALUE)
...
LONG WINAPI unhandled_handler(struct _EXCEPTION_POINTERS* exceptionInfo)
{
	...
	if (pRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION)
	{
		...
		{
			sprintf(tmp, "%s (0x%x)\nAddress: %p\n\n\nThe thread attempted to write to an inaccessible address %p\nMinidump 'PsyCrossCrash.dmp' will be saved.", 
				pName, pRecord->ExceptionCode, pRecord->ExceptionAddress, inaccessibleAddress);
		}
		else
		{
	...
	VirtualQuery(pRecord->ExceptionAddress, &memoryInfo, sizeof(memoryInfo));
	FILE* crashLog = fopen("PsyCrossCrash.txt", "w");
	if (crashLog != NULL)
	{
		const uintptr_t address = reinterpret_cast<uintptr_t>(pRecord->ExceptionAddress);
		const uintptr_t moduleBase = reinterpret_cast<uintptr_t>(memoryInfo.AllocationBase);
		fprintf(crashLog,
			"code=0x%08lx address=%p module_base=%p rva=0x%llx access_kind=%llu access_address=%p\n",
			pRecord->ExceptionCode, pRecord->ExceptionAddress,
			memoryInfo.AllocationBase,
			static_cast<unsigned long long>(address - moduleBase),
			static_cast<unsigned long long>(pRecord->ExceptionInformation[0]),
			pRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION
				? reinterpret_cast<void*>(pRecord->ExceptionInformation[1]) : NULL);
		fclose(crashLog);
	}
