...
static int PsyX_IsInvalidLogFilenameCharacter(unsigned char character)
{
...
{
	char appLogFilename[1024];
	const char* explicitLogFilename = getenv("PSYX_LOG_FILE");
	if (explicitLogFilename != NULL && explicitLogFilename[0] != '\0')
	{
		snprintf(appLogFilename, sizeof(appLogFilename), "%s", explicitLogFilename);
	}
	else
	{
		const char* sourceName = g_appNameStr != NULL ? g_appNameStr : "PsyX";
		size_t nameLength = 0;
		while (sourceName[nameLength] != '\0' &&
		       nameLength + sizeof(".log") < sizeof(appLogFilename))
		{
			const unsigned char character = (unsigned char)sourceName[nameLength];
			appLogFilename[nameLength] =
			    PsyX_IsInvalidLogFilenameCharacter(character) ? '_' : (char)character;
			nameLength++;
		}
		memcpy(appLogFilename + nameLength, ".log", sizeof(".log"));
	}
