...
void DrawOTag(u_long* p)
{
	do
	{
		...
		ParsePrimitivesLinkedList(p, 0, 0);
...
void DrawPrim(void* p)
{
	...
	ParsePrimitivesLinkedList((u_long*)p, 1, 0);
}

#if USE_PGXP
void DrawPrimPGXP(void* p, u_short pgxp_index)
{
	if (g_GPUDisabledState)
	{
		ClearSplits();
...
#if USE_EXTENDED_PRIM_POINTERS
	((P_TAG*)p)->pgxp_index = pgxp_index;
#endif
	ParsePrimitivesLinkedList((u_long*)p, 1, 1);
}
#endif
