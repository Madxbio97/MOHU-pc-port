...
int main() {
  ...
  PsyCrossGuestGpu reset_epoch_gpu;
  constexpr std::array<std::uint32_t, 2U> incomplete_triangle{
      0x20000000U, 0x00000000U};
  reset_epoch_gpu.submit(incomplete_triangle, {}, {}, 0U);
  if (reset_epoch_gpu.submittedCommands() != 0U) {
    std::cerr << "Incomplete GP0 triangle was submitted\n";
    return 10;
  }
  reset_epoch_gpu.submit(std::span<const std::uint32_t>{}, {}, {}, 1U);
  constexpr std::array<std::uint32_t, 1U> nop{0x00000000U};
  reset_epoch_gpu.submit(nop, {}, {}, 1U);
  if (reset_epoch_gpu.submittedCommands() != 1U ||
      reset_epoch_gpu.unsupportedCommands() != 0U) {
    std::cerr << "GP1 reset epoch retained an incomplete GP0 packet\n";
    return 11;
  }
