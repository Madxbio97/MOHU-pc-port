...
int main() {
  ...
  clipped_handle_gpu.setDiagnosticGeometryOptions(true, true, true, true, true,
                                                  true, true);
  ...
  close_z_gpu.setDiagnosticGeometryOptions(true, true, true, true, false, false,
                                           true);
  ...
  GR_BeginGuestSubmit();
  PGXP_ClearCache();
  sf::platform::detail::PsyCrossGuestGpu native_gpu;
  constexpr std::array<std::uint32_t, 5U> native_state_source{
      0xe1000000U, 0xe2000000U, 0xe3000000U, 0xe407ffffU, 0xe5000000U,
  };
  native_gpu.submit(native_state_source);
  constexpr std::array<std::uint32_t, 7U> native_source{
      0x24010203U, 0x000a000aU, 0x0000140aU, 0x000a001eU,
      0x0001281eU, 0x0028000aU, 0x00003c0aU,
  };
  const std::array<sf::psx::NativeGpuPosition, 3U> native_positions{{
      {.view_x = 10.0F, .view_y = 10.0F, .view_z = 64.0F, .screen_h = 64.0F},
      {.view_x = 30.0F, .view_y = 10.0F, .view_z = 64.0F, .screen_h = 64.0F},
      {.view_x = 10.0F, .view_y = 40.0F, .view_z = 64.0F, .screen_h = 64.0F},
  }};
  const std::array<sf::psx::NativeGpuTriangle, 1U> native_triangles{{
      {.vertices = {{
           {.u = 10.25F,
            .v = 20.5F,
            .red = 255.0F,
       ...
       .material = {.clut = 0U, .texture_page = 1U, .textured = true},
       .source_word_offset = 0U,
       .source_triangle_count = 1U,
       .texture_bounds = 0x3c281e0aU,
       .source_word_count = 7U,
       .source_opcode = 0x24U},
  }};
  native_gpu.setNativeScene(native_positions, native_triangles);
  native_gpu.submit(native_source);
  PGXPVData native_first{};
  PGXPVData native_last{};
  if (native_gpu.nativeScenePrimitives() != 1U ||
      native_gpu.nativeSceneTriangles() != 1U ||
      native_gpu.nativeSceneFallbacks() != 0U ||
      native_gpu.nativeSceneStateFallbacks() != 0U ||
      native_gpu.precisePrimitives() != 1U ||
      native_gpu.polygonPrimitives() != 1U || PGXP_GetIndex(0) != 3U ||
      PGXP_GetCacheDataExact(&native_first, 0U) == 0 ||
      PGXP_GetCacheDataExact(&native_last, 2U) == 0 ||
      native_first.exact_projection == 0U || native_first.pz != 0.5F ||
      native_first.precise_u != 10.25F || native_first.precise_v != 20.5F ||
      native_last.precise_u != 10.75F ||
      native_last.texture_bounds != 0x3c281e0aU) {
    std::cerr << "Direct native triangle lost view-space W, UV, or coverage\n";
    PsyX_Shutdown();
    return 80;
  }

  std::array<sf::psx::GteProjectedVertex, native_source.size()>
      native_hazard_projections{};
  constexpr std::array<std::size_t, 3U> native_coordinate_words{1U, 3U, 5U};
  for (std::size_t vertex{}; vertex < native_coordinate_words.size();
       ++vertex) {
    const auto word = native_coordinate_words[vertex];
    auto &projection = native_hazard_projections[word];
    const auto &position = native_positions[vertex];
    projection.packed_sxy = native_source[word];
    projection.view_x = position.view_x;
    projection.view_y = position.view_y;
    projection.view_z = position.view_z;
    projection.projective_depth = position.view_z;
    projection.screen_x = position.view_x;
    projection.screen_y = position.view_y;
    projection.screen_h = position.screen_h;
    projection.screen_saturated = true;
    projection.valid = true;
  }
  PGXP_ClearCache();
  sf::platform::detail::PsyCrossGuestGpu native_hazard_gpu;
  native_hazard_gpu.submit(native_state_source);
  native_hazard_gpu.setNativeScene(native_positions, native_triangles);
  native_hazard_gpu.submit(native_source, native_hazard_projections);
  if (native_hazard_gpu.nativeScenePrimitives() != 0U ||
      native_hazard_gpu.nativeSceneTriangles() != 0U ||
      native_hazard_gpu.nativeSceneFallbacks() != 1U ||
      native_hazard_gpu.nativeSceneStateFallbacks() != 0U ||
      native_hazard_gpu.precisePrimitives() != 0U ||
      native_hazard_gpu.polygonPrimitives() != 1U ||
      native_hazard_gpu.screenSaturationPrimitives() != 1U ||
      native_hazard_gpu.submittedCommands() != 6U) {
    std::cerr << "Hazardous projection bypassed native fail-closed fallback\n";
    PsyX_Shutdown();
    return 102;
  }

  auto incomplete_triangle_batch = native_triangles[0U];
  incomplete_triangle_batch.source_triangle_count = 2U;
  PGXP_ClearCache();
  sf::platform::detail::PsyCrossGuestGpu incomplete_native_gpu;
  incomplete_native_gpu.submit(native_state_source);
  incomplete_native_gpu.setNativeScene(
      native_positions, std::span<const sf::psx::NativeGpuTriangle>{
                            &incomplete_triangle_batch, 1U});
  incomplete_native_gpu.submit(native_source);
  if (incomplete_native_gpu.nativeScenePrimitives() != 0U ||
      incomplete_native_gpu.nativeSceneTriangles() != 0U ||
      incomplete_native_gpu.nativeSceneFallbacks() != 1U ||
      incomplete_native_gpu.nativeSceneStateFallbacks() != 0U ||
      incomplete_native_gpu.precisePrimitives() != 0U ||
      incomplete_native_gpu.polygonPrimitives() != 1U ||
      incomplete_native_gpu.submittedCommands() != 6U) {
    std::cerr << "Incomplete native triangle batch did not use raw fallback\n";
    PsyX_Shutdown();
    return 101;
  }

  const std::array<sf::psx::NativeGpuPosition, 3U> native_depth_positions{{
      {.view_x = 1.25F, .view_y = 1.25F, .view_z = 40.0F, .screen_h = 320.0F},
      {.view_x = 7.5F, .view_y = 2.5F, .view_z = 80.0F, .screen_h = 320.0F},
      {.view_x = 6.25F, .view_y = 25.0F, .view_z = 200.0F, .screen_h = 320.0F},
  }};
  PGXP_ClearCache();
  sf::platform::detail::PsyCrossGuestGpu native_depth_gpu;
  native_depth_gpu.submit(native_state_source);
  native_depth_gpu.setNativeScene(native_depth_positions, native_triangles);
  native_depth_gpu.submit(native_source);
  std::array<PGXPVData, 3U> native_depth_cache{};
  auto native_depth_exact = true;
  for (std::size_t vertex{}; vertex < native_depth_cache.size(); ++vertex) {
    native_depth_exact =
        native_depth_exact &&
        PGXP_GetCacheDataExact(&native_depth_cache[vertex],
                               static_cast<u_short>(vertex)) != 0 &&
        native_depth_cache[vertex].exact_projection != 0U &&
        native_depth_cache[vertex].scr_h == 320.0F;
  }
  if (native_depth_gpu.nativeScenePrimitives() != 1U ||
      native_depth_gpu.nativeSceneTriangles() != 1U ||
      native_depth_gpu.nativeSceneFallbacks() != 0U || !native_depth_exact ||
      native_depth_cache[0U].pz != 40.0F / 128.0F ||
      native_depth_cache[1U].pz != 80.0F / 128.0F ||
      native_depth_cache[2U].pz != 200.0F / 128.0F) {
    std::cerr << "Native exact path replaced view-space W with clamped SZ\n";
    PsyX_Shutdown();
    return 100;
  }
  auto unsupported_state_triangle = native_triangles[0U];
  unsupported_state_triangle.draw_state.texture_window = 1U;
  unsupported_state_triangle.source_word_offset = 1U;
  constexpr std::array<std::uint32_t, 8U> unsupported_state_source{
      0xe2000001U, 0x24010203U, 0x000a000aU, 0x0000140aU,
      0x000a001eU, 0x0001281eU, 0x0028000aU, 0x00003c0aU,
  };
  sf::platform::detail::PsyCrossGuestGpu unsupported_state_gpu;
  unsupported_state_gpu.submit(native_state_source);
  unsupported_state_gpu.setNativeScene(
      native_positions, std::span<const sf::psx::NativeGpuTriangle>{
                            &unsupported_state_triangle, 1U});
  unsupported_state_gpu.submit(unsupported_state_source);
  if (unsupported_state_gpu.nativeScenePrimitives() != 0U ||
      unsupported_state_gpu.nativeSceneFallbacks() != 1U ||
      unsupported_state_gpu.nativeSceneStateFallbacks() != 1U ||
      unsupported_state_gpu.submittedCommands() != 7U) {
    std::cerr << "Unsupported native draw state did not fall back atomically\n";
    PsyX_Shutdown();
    return 97;
  }

  auto semitrans_triangle = native_triangles[0U];
  semitrans_triangle.source_opcode = 0x26U;
  semitrans_triangle.material.semi_transparent = true;
  auto semitrans_source = native_source;
  semitrans_source[0U] = (semitrans_source[0U] & 0x00ffffffU) | 0x26000000U;
  sf::platform::detail::PsyCrossGuestGpu semitrans_native_gpu;
  semitrans_native_gpu.submit(native_state_source);
  semitrans_native_gpu.setNativeScene(
      native_positions,
      std::span<const sf::psx::NativeGpuTriangle>{&semitrans_triangle, 1U});
  semitrans_native_gpu.submit(semitrans_source);
  if (semitrans_native_gpu.nativeScenePrimitives() != 0U ||
      semitrans_native_gpu.nativeSceneFallbacks() != 1U ||
      semitrans_native_gpu.nativeSceneStateFallbacks() != 1U ||
      semitrans_native_gpu.polygonPrimitives() != 1U) {
    std::cerr << "Semitransparent native effect bypassed legacy blending\n";
    PsyX_Shutdown();
    return 98;
  }

  auto interleaved_triangle = native_triangles[0U];
  interleaved_triangle.source_word_offset = 10U;
  constexpr std::array<std::uint32_t, 17U> interleaved_source{
      0xe1000000U, 0xe2000000U, 0xe3000000U, 0xe407ffffU, 0xe5000000U,
      0xe6000000U, 0x64808080U, 0x00010001U, 0x00000000U, 0x00020002U,
      0x24010203U, 0x000a000aU, 0x0000140aU, 0x000a001eU, 0x0001281eU,
      0x0028000aU, 0x00003c0aU,
  };
  sf::platform::detail::PsyCrossGuestGpu interleaved_native_gpu;
  interleaved_native_gpu.setNativeScene(
      native_positions,
      std::span<const sf::psx::NativeGpuTriangle>{&interleaved_triangle, 1U});
  interleaved_native_gpu.submit(interleaved_source);
  if (interleaved_native_gpu.nativeScenePrimitives() != 1U ||
      interleaved_native_gpu.nativeSceneTriangles() != 1U ||
      interleaved_native_gpu.nativeSceneFallbacks() != 0U ||
      interleaved_native_gpu.submittedCommands() != 8U ||
      interleaved_native_gpu.nativeSceneStateFallbacks() != 0U ||
      interleaved_native_gpu.polygonPrimitives() != 1U) {
    std::cerr << "Legacy sprite interleave disrupted native polygon dispatch\n";
    PsyX_Shutdown();
    return 99;
  }

  constexpr std::array<std::uint32_t, 5U> native_quad_source{
      0x28010203U, 0x000a000aU, 0x000a001eU, 0x0028000aU, 0x0028001eU,
  };
  std::array<sf::psx::NativeGpuTriangle, 4U> native_quad_triangles{};
  for (std::size_t index{}; index < native_quad_triangles.size(); ++index) {
    auto &triangle = native_quad_triangles[index];
    triangle.vertices = {{
        {.red = 255.0F, .green = 255.0F, .blue = 255.0F, .position_index = 0U},
        {.red = 255.0F, .green = 255.0F, .blue = 255.0F, .position_index = 1U},
        {.red = 255.0F, .green = 255.0F, .blue = 255.0F, .position_index = 2U},
    }};
    triangle.source_triangle_ordinal = static_cast<std::uint8_t>(index);
    triangle.source_triangle_count =
        static_cast<std::uint8_t>(native_quad_triangles.size());
    triangle.source_word_count = 5U;
    triangle.source_opcode = 0x28U;
  }
  PGXP_ClearCache();
  sf::platform::detail::PsyCrossGuestGpu native_quad_gpu;
  native_quad_gpu.submit(native_state_source);
  native_quad_gpu.setNativeScene(native_positions, native_quad_triangles);
  native_quad_gpu.submit(native_quad_source);
  if (native_quad_gpu.nativeScenePrimitives() != 1U ||
      native_quad_gpu.nativeSceneTriangles() != 4U ||
      native_quad_gpu.nativeSceneFallbacks() != 0U || PGXP_GetIndex(0) != 12U) {
    std::cerr << "Four-triangle native quad did not replace atomically\n";
    PsyX_Shutdown();
    return 96;
  }
  DrawSync(0);
