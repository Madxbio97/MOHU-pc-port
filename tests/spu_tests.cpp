...
constexpr std::uint32_t voice_current_adsr = 0x00cU;
...
constexpr std::uint32_t key_off_low = 0x18cU;
...
constexpr std::uint32_t sample_ram_base = 0x1000U;
constexpr std::uint16_t sample_address_units =
    static_cast<std::uint16_t>(sample_ram_base >> 3U);
...
void keyOnVoiceZero(sf::psx::Spu &spu, std::uint16_t pitch = 0U) {
  ...
  require(spu.writeRegister(voice_repeat_address, sample_address_units),
  ...
  // KON is sampled after the current 44.1 kHz boundary.
  spu.mixFrames(1U);
  spu.clearPcm();
}
...
void testKeyBoundaryAndCaptureBuffers() {
  auto spu = std::make_unique<sf::psx::Spu>();
  require(spu->writeRegister(control, control_enable) &&
              spu->writeRegister(key_on_low, 1U),
          "Could not queue SPU KON boundary test");
  ...
  require(spu->state().voices[0].active == 1U &&
              ...
              spu->state().pending_key_on == 0U,
          "SPU did not apply pending KON after one sample");

  require(spu->writeRegister(key_off_low, 1U) &&
              spu->writeRegister(key_on_low, 1U),
          "Could not queue simultaneous SPU KOFF/KON");
  spu->mixFrames(1U);
  require(spu->state().voices[0].active == 1U &&
              spu->state().voices[0].adsr_phase ==
                  sf::psx::SpuAdsrPhase::attack &&
              spu->state().pending_key_on == 0U &&
              spu->state().pending_key_off == 0U,
          "Simultaneous KOFF/KON did not restart the voice in hardware order");

  auto capture = std::make_unique<sf::psx::Spu>();
  const std::array cd{sf::psx::SpuPcmFrame{0x1234, -0x2345}};
  require(capture->writeRegister(irq_address, 0U) &&
              capture->writeRegister(control, control_irq_enable) &&
              capture->pushCdAudio(cd) == cd.size(),
          "Could not configure SPU capture/IRQ test");
  capture->mixFrames(1U);
  const auto read_capture = [&capture](std::size_t address) {
    return static_cast<std::uint16_t>(
        std::to_integer<std::uint8_t>(capture->ram()[address]) |
        (std::to_integer<std::uint8_t>(capture->ram()[address + 1U]) << 8U));
  };
  ...
  capture->mixFrames(255U);
  std::uint16_t status_value{};
  require(capture->state().capture_buffer_position == 0x200U &&
...
void testAdpcmFilterVectors() {
  {
    ...
    writeRamBytes(*spu, sample_ram_base, block);
    keyOnVoiceZero(*spu);
    spu->mixFrames(1U);
...
void testHardwarePitchClampAndPitchModulation() {
...

  {
    ...
    require(spu->writeRegister(0x00cU, 0x7fffU) &&
                spu->writeRegister(0x01cU, 0x7fffU),
            "Could not seed post-KON pitch-modulation envelopes");
...
void testHardwareVolumeSweeps() {
  ...
  // A decreasing sweep must remain live until it reaches zero. The old
  // inverted completion predicate stopped after the first non-zero step,
  // leaving transition/menu voices stuck at an audible volume.
  require(spu->writeRegister(main_volume_left, 0x1000U) &&
              spu->writeRegister(main_volume_left, 0xa008U),
          "Could not configure a decreasing SPU main-volume sweep");
  spu->mixFrames(1U);
  require(spu->readRegister(current_main_volume_left, current) &&
              current == 0x1000U &&
              spu->state().main_volume[0U].envelope_active == 1U,
          "SPU decreasing volume sweep stopped before reaching zero");
  spu->mixFrames(1U);
  require(spu->readRegister(current_main_volume_left, current) &&
              current == 0U &&
              spu->state().main_volume[0U].envelope_active == 0U,
          "SPU decreasing volume sweep remained active after reaching zero");
  spu->mixFrames(1U);
  require(spu->readRegister(current_main_volume_left, current) && current == 0U,
          "Stopped SPU decreasing volume sweep moved below zero");

  // Phase-inverted decreasing sweeps approach zero from the negative side.
  // Keep this branch separate from the ordinary decreasing case above: its
  // step direction and completion predicate are both inverted.
  require(spu->writeRegister(main_volume_left, 0x7000U) &&
              spu->writeRegister(main_volume_left, 0xb008U),
          "Could not configure a phase-inverted decreasing SPU sweep");
  spu->mixFrames(1U);
  require(spu->readRegister(current_main_volume_left, current) &&
              current == 0xee00U &&
              spu->state().main_volume[0U].envelope_active == 1U,
          "Phase-inverted decreasing SPU sweep stopped while negative");
  spu->mixFrames(1U);
  require(spu->readRegister(current_main_volume_left, current) &&
              current == 0xfc00U &&
              spu->state().main_volume[0U].envelope_active == 1U,
          "Phase-inverted decreasing SPU sweep used the wrong direction");
  spu->mixFrames(1U);
  require(spu->readRegister(current_main_volume_left, current) &&
              current == 0U &&
              spu->state().main_volume[0U].envelope_active == 0U,
          "Phase-inverted decreasing SPU sweep did not stop at zero");
  spu->mixFrames(1U);
  require(spu->readRegister(current_main_volume_left, current) && current == 0U,
          "Stopped phase-inverted decreasing SPU sweep moved above zero");
}
...
void testAdpcmLoopAndEndx() {
  ...
  spu->mixFrames(28U);
  require(spu->endx() == 0U &&
              spu->state().voices[0].block_address == sample_ram_base + 16U &&
              spu->state().voices[0].repeat_address == sample_ram_base,
          "SPU did not retain the ADPCM loop-start address");
  spu->mixFrames(28U);
  require((spu->endx() & 1U) != 0U && spu->state().voices[0].active == 1U &&
              spu->state().voices[0].block_address == sample_ram_base,
...
void testLoopedVoiceKeyOffRelease() {
  auto spu = std::make_unique<sf::psx::Spu>();
  std::array<std::byte, 16U> block{};
  block[0U] = std::byte{0x0cU};
  block[1U] = std::byte{0x07U};
  std::fill(block.begin() + 2U, block.end(), std::byte{0x77U});
  writeRamBytes(*spu, sample_ram_base, block);

  require(spu->writeRegister(voice_volume_left, 0x3fffU) &&
              spu->writeRegister(voice_volume_right, 0x3fffU) &&
              spu->writeRegister(main_volume_left, 0x3fffU) &&
              spu->writeRegister(main_volume_right, 0x3fffU) &&
              spu->writeRegister(voice_adsr_high, 0x0008U),
          "Could not configure the looped KEY-OFF test voice volumes");
  keyOnVoiceZero(*spu, 0x1000U);
  require(spu->writeRegister(voice_current_adsr, 0x7fffU) &&
              spu->writeRegister(key_off_low, 1U),
          "Could not enter the looped voice release phase");
  spu->mixFrames(1U);
  spu->clearPcm();
  ...
  require(spu->state().voices[0].active == 1U &&
              spu->state().voices[0].adsr_phase ==
                  sf::psx::SpuAdsrPhase::release &&
              spu->state().voices[0].envelope == 63U,
          "SPU KEY-OFF release ended before its programmed duration");
  spu->mixFrames(1U);
  require(spu->state().voices[0].active == 0U &&
              spu->state().voices[0].adsr_phase == sf::psx::SpuAdsrPhase::off &&
              spu->state().voices[0].envelope == 0U,
          "SPU KEY-OFF left a looped transition voice active");

  const auto stopped_block = spu->state().voices[0].block_address;
  const auto stopped_sample = spu->state().voices[0].sample_index;
  spu->clearPcm();
  spu->mixFrames(64U);
  std::array<sf::psx::SpuPcmFrame, 64U> silence{};
  require(spu->takePcm(silence) == silence.size() &&
              std::ranges::all_of(silence,
                                  [](const auto &frame) {
                                    return frame == sf::psx::SpuPcmFrame{};
                                  }) &&
              spu->state().voices[0].block_address == stopped_block &&
              spu->state().voices[0].sample_index == stopped_sample,
          "Stopped looped voice advanced or emitted stale PCM");
}
...
void testAdpcmAddressMaskAndLateLoopRegisterWrite() {
  ...
  require(spu->state().voices[0].repeat_address == sample_ram_base + 32U &&
              spu->state().voices[0].ignore_loop_address == 1U &&
              spu->readRegister(voice_repeat_address, repeat_register) &&
              repeat_register == sample_address_units + 4U,
