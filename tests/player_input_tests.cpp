...
expectedPcInputForAction(sf::platform::KeyboardMouseAction action) {
  ...
  case KeyboardMouseAction::strafe_right:
    ...
    break;
  case KeyboardMouseAction::lean_left:
    expected.lean_left = true;
    break;
  case KeyboardMouseAction::lean_right:
    expected.lean_right = true;
    break;
...
bool mappedActionMatches(const sf::platform::PlayerInput &input,
  ...
  case KeyboardMouseAction::strafe_right:
    return near(input.move_strafe, 1.0);
  case KeyboardMouseAction::lean_left:
    return near(input.move_peek, -1.0);
  case KeyboardMouseAction::lean_right:
    return near(input.move_peek, 1.0);
  case KeyboardMouseAction::run:
...
void testKeyboardMouseBindingCatalog() {
  ...
  require(sf::platform::keyboard_mouse_action_count == 33U &&
              sf::platform::keyboardMouseActionCatalog().size() == 33U,
  ...
  for (std::size_t index = 0U;
    ...
    const auto &metadata = sf::platform::keyboardMouseActionCatalog()[index];
    require(metadata.action == action &&
                metadata.default_input == defaults[action] &&
                !sf::platform::keyboardMouseActionName(action).empty() &&
  ...
  require(
      defaults[KeyboardMouseAction::move_forward] == KeyboardMouseInput::w &&
          defaults[KeyboardMouseAction::strafe_left] == KeyboardMouseInput::a &&
          defaults[KeyboardMouseAction::strafe_right] ==
              KeyboardMouseInput::d &&
          defaults[KeyboardMouseAction::lean_left] == KeyboardMouseInput::q &&
          defaults[KeyboardMouseAction::lean_right] == KeyboardMouseInput::e &&
          ...
          defaults[KeyboardMouseAction::crouch] ==
              KeyboardMouseInput::left_control &&
          defaults[KeyboardMouseAction::reload] == KeyboardMouseInput::r &&
...
void testMohUndergroundRuntimePadMapping() {
  using sf::platform::KeyboardMouseAction;
  using sf::platform::KeyboardMouseInput;
  ...
  RuntimePadInput physical{
      .active_low_buttons = 0xfffeU,
      .analog = {17U, 34U, 99U, 88U},
  };
  sf::platform::KeyboardMouseActionSnapshot actions;
  const auto hold = [&actions](KeyboardMouseAction action) {
    actions.held[static_cast<std::size_t>(action)] = true;
  };
  hold(KeyboardMouseAction::move_forward);
  hold(KeyboardMouseAction::turn_right);
  hold(KeyboardMouseAction::strafe_left);
  hold(KeyboardMouseAction::strafe_right);
  hold(KeyboardMouseAction::roll);
  hold(KeyboardMouseAction::reload);
  hold(KeyboardMouseAction::aim);
  hold(KeyboardMouseAction::fire);
  hold(KeyboardMouseAction::crouch);
  hold(KeyboardMouseAction::interact);
  hold(KeyboardMouseAction::pause);
  hold(KeyboardMouseAction::next_weapon);

  const auto mapped = sf::platform::mergeMohUndergroundRuntimeInput(
      physical, actions, runtime_actions);
  constexpr std::uint16_t keyboard_pressed =
      0x0002U | 0x0008U | 0x0010U | 0x0020U | 0x0200U | 0x0400U | 0x0800U |
      0x1000U | 0x2000U | 0x4000U | 0x8000U;
  require(mapped.active_low_buttons ==
              static_cast<std::uint16_t>(physical.active_low_buttons &
                                         ~keyboard_pressed),
          "MOHU keyboard actions reached the wrong active-low PS1 buttons");
  require(mapped.analog == physical.analog,
          "MOHU keyboard movement duplicated menu navigation through "
          "analog axes");

  auto default_keyboard = std::array<std::uint8_t, 32U>{};
  default_keyboard[static_cast<std::size_t>(KeyboardMouseInput::r)] = 1U;
  const auto default_actions = sf::platform::sampleKeyboardMouseActions(
      sf::platform::defaultKeyboardMouseBindings(),
      sf::platform::KeyboardMouseDeviceState{
          .keyboard = default_keyboard,
          .mouse_left = true,
          .mouse_right = true,
      });
  const auto default_combat = sf::platform::mergeMohUndergroundRuntimeInput(
      RuntimePadInput{}, default_actions, runtime_actions);
  constexpr std::uint16_t default_combat_buttons = 0x0200U | 0x4000U | 0x8000U;
  require(default_combat.active_low_buttons ==
              static_cast<std::uint16_t>(0xffffU & ~default_combat_buttons),
          "R, LMB, or RMB did not reach reload, fire, and aim respectively");

  sf::platform::KeyboardMouseActionSnapshot opposed;
  for (const auto action :
       {KeyboardMouseAction::move_forward, KeyboardMouseAction::move_backward,
        KeyboardMouseAction::turn_left, KeyboardMouseAction::turn_right}) {
    opposed.held[static_cast<std::size_t>(action)] = true;
  }
  const auto canceled = sf::platform::mergeMohUndergroundRuntimeInput(
      physical, opposed, runtime_actions);
  require(canceled.analog == physical.analog,
          "Opposing MOHU keyboard movement changed physical pad axes");
  require((canceled.active_low_buttons & 0x00f0U) == 0U,
          "MOHU menu D-pad did not receive all held movement directions");

  const auto defaults = sf::platform::defaultKeyboardMouseBindings();
  const auto wheel_up = sf::platform::sampleKeyboardMouseActions(
      defaults, sf::platform::KeyboardMouseDeviceState{.mouse_wheel_delta = 2});
  const auto wheel_down = sf::platform::sampleKeyboardMouseActions(
      defaults,
      sf::platform::KeyboardMouseDeviceState{.mouse_wheel_delta = -2});
  const auto wheel_up_pad = sf::platform::mergeMohUndergroundRuntimeInput(
      RuntimePadInput{}, wheel_up, runtime_actions);
  const auto wheel_down_pad = sf::platform::mergeMohUndergroundRuntimeInput(
      RuntimePadInput{}, wheel_down, runtime_actions);
  require(wheel_up_pad.active_low_buttons == 0xdfffU &&
              wheel_down_pad.active_low_buttons == 0xdfffU,
          "Both MOHU mouse-wheel directions must reach the Circle weapon "
          "action");

  require(sf::platform::mergeMohUndergroundRuntimeInput(
              physical, {}, runtime_actions) == physical,
...
void testMohUndergroundRuntimeMouseLook() {
  ...
  auto analog_actions =
      sf::platform::defaultMohUndergroundRuntimeActionBindings();
  analog_actions.mouse_look_mode =
      sf::platform::MohUndergroundMouseLookMode::right_stick;

  require(sf::platform::applyMohUndergroundRuntimeMouseLook(
              neutral, 10, -8, false, 100U, analog_actions) == neutral,
          "MOHU mouse look moved the stick while capture was inactive");
  ...
  const auto saturated = sf::platform::applyMohUndergroundRuntimeMouseLook(
      neutral, 100, -100, true, 100U, analog_actions);
  require(saturated.analog ==
              std::array<std::uint8_t, 4U>{255U, 0U, 128U, 128U},
          "MOHU mouse look did not saturate at DualShock axis limits");

  RuntimePadInput physical{
      .active_low_buttons = 0xfffeU,
      .analog = {140U, 120U, 99U, 88U},
  };
  ...
  const auto extremes = sf::platform::applyMohUndergroundRuntimeMouseLook(
      neutral, std::numeric_limits<std::int32_t>::min(),
      std::numeric_limits<std::int32_t>::max(), true, 400U, analog_actions);
  require(extremes.analog == std::array<std::uint8_t, 4U>{0U, 255U, 128U, 128U},
          "MOHU mouse look overflowed on extreme SDL deltas");

  const auto directional_actions =
      sf::platform::defaultMohUndergroundRuntimeActionBindings();
  RuntimePadInput aiming;
  aiming.analog = {150U, 90U, 128U, 128U};
  aiming.active_low_buttons =
      static_cast<std::uint16_t>(aiming.active_low_buttons & ~0x0200U);
  const auto aimed = sf::platform::applyMohUndergroundRuntimeMouseLook(
      aiming, 10, -8, true, 100U, directional_actions);
  ...
  require(released == neutral,
          "MOHU chase camera did not return its virtual stick to center");

  require(sf::platform::runtimeMouseDeltaForGuestStep(10, 0U, 3U) == 4 &&
              sf::platform::runtimeMouseDeltaForGuestStep(10, 1U, 3U) == 3 &&
              sf::platform::runtimeMouseDeltaForGuestStep(10, 2U, 3U) == 3 &&
              sf::platform::runtimeMouseDeltaForGuestStep(-10, 0U, 3U) == -4 &&
              sf::platform::runtimeMouseDeltaForGuestStep(-10, 1U, 3U) == -3 &&
              sf::platform::runtimeMouseDeltaForGuestStep(-10, 2U, 3U) == -3,
          "MOHU catch-up steps did not preserve signed mouse movement");

  auto extreme_sum = std::int64_t{};
  for (auto step = std::size_t{}; step < 4U; ++step) {
    extreme_sum += sf::platform::runtimeMouseDeltaForGuestStep(
        std::numeric_limits<std::int32_t>::min(), step, 4U);
  }
  require(extreme_sum == std::numeric_limits<std::int32_t>::min(),
          "MOHU catch-up mouse split overflowed an extreme SDL delta");
  require(sf::platform::runtimeMouseDeltaForGuestStep(5, 0U, 0U) == 0 &&
              sf::platform::runtimeMouseDeltaForGuestStep(5, 3U, 3U) == 0,
          "MOHU catch-up mouse split accepted an invalid guest step");
}
...
void testControllerCameraDeadzoneRejectsDualSenseDrift() {
  ...
  const auto deliberate = sf::platform::firstPersonAimInput(mapper.update(raw));
...
void testOriginalControllerActions() {
  ...
  require(near(output.move_peek, -1.0),
          "Original L2 lean-left action was not mapped");
}
...
void testPcAimAndOpposingControlConflicts() {
  ...
  require(near(chase.turn, 0.0) && near(chase.move_strafe, -1.0),
          "A did not strafe left while mouse look owned turning");
  ...
  require(near(aiming.move_forward, 1.0) && near(aiming.turn, 0.0) &&
              near(aiming.move_strafe, -1.0),
