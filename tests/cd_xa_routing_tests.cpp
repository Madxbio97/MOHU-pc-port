...
void testMachineStopClearsXaTimeline() {
  ...
  require(machine->spu().queuedCdFrames() != 0U &&
              machine->captureState().xa_decoder.active != 0U,
          "Machine XA stream did not establish a queued timeline");

  machineExecuteCommand(*runtime, *machine, command_stop);
  const auto stopped = machine->captureState();
  const auto reset_decoder = sf::psx::XaAudioDecoder{}.captureState();
  require(stopped.spu != nullptr && stopped.spu->cd_frame_count == 0U &&
              stopped.xa_decoder == reset_decoder &&
              stopped.cdrom.reading == 0U &&
              stopped.cdrom.sector_event.pending == 0U &&
              stopped.cdrom.xa_current_set == 0U,
          "CD Stop retained stale XA decoder or SPU queue state");
}
...
int main() {
  try {
    ...
    testMachineStopClearsXaTimeline();
