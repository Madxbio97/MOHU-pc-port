...
void writeTrack(const std::filesystem::path &path) {
  ...
  std::fill_n(bytes.begin(), sf::disc::RawSectorSource::raw_sector_size,
              std::byte{0x09});
  std::fill_n(bytes.begin() + sf::disc::RawSectorSource::raw_sector_size,
              sf::disc::RawSectorSource::raw_sector_size, std::byte{0x11});
  std::fill_n(bytes.begin() + 2U * sf::disc::RawSectorSource::raw_sector_size,
              sf::disc::RawSectorSource::raw_sector_size, std::byte{0x22});
...
void writePatternedTrack(const std::filesystem::path &path,
...
void requirePatternedSector(std::span<const std::byte> bytes,
                            std::uint32_t logical_lba, const char *message) {
  require(bytes.size() == sf::disc::RawSectorSource::raw_sector_size &&
              std::ranges::all_of(
                  bytes, [expected = patternedSectorValue(logical_lba)](
                             std::byte value) { return value == expected; }),
          message);
}

void writeCue(const std::filesystem::path &path,
...
void testStreamingAndBounds() {
  ...
  require(source.sectorCount() == 2U,
          "CUE INDEX was not excluded from logical LBAs");
  ...
  require(both.front() == std::byte{0x11},
          "Bulk raw read lost its first sector");
...
void testUserDataOverlay() {
  sf::test::TemporaryDirectory temporary{"sf_raw_sector_overlay_tests"};
  const auto cue_path = temporary.path() / "disc.cue";
  writeTrack(temporary.path() / "track.bin");
  writeCue(cue_path);

  auto source = sf::disc::RawSectorSource::open(cue_path);
  std::vector<std::byte> overlay(2048U + 17U, std::byte{0xa1});
  std::fill(overlay.begin() + 2048U, overlay.end(), std::byte{0xb2});
  const std::vector<std::byte> empty;
  const std::array one_byte{std::byte{0x7f}};
  require(!source.addUserDataOverlay(0U, empty),
          "Raw overlay accepted empty data");
  require(source.addUserDataOverlay(0U, overlay),
          "Raw overlay rejected a valid two-sector extent");
  require(!source.addUserDataOverlay(1U, one_byte),
          "Raw overlay accepted an overlapping extent");
  require(!source.addUserDataOverlay(2U, one_byte),
          "Raw overlay crossed the end of the track");

  const auto first = source.readSector(0U);
  require(std::ranges::all_of(
              first.begin() + 24U, first.begin() + 24U + 2048U,
              [](std::byte value) { return value == std::byte{0xa1}; }) &&
              std::ranges::all_of(
                  first.begin(), first.begin() + 24U,
                  [](std::byte value) { return value == std::byte{0x11}; }) &&
              std::ranges::all_of(
                  first.begin() + 24U + 2048U, first.end(),
                  [](std::byte value) { return value == std::byte{0x11}; }),
          "Raw overlay changed bytes outside the first user-data payload");

  std::vector<std::byte> both(2U * sf::disc::RawSectorSource::raw_sector_size);
  source.readSectors(0U, both);
  const auto second = both.begin() + sf::disc::RawSectorSource::raw_sector_size;
  require(std::ranges::all_of(
              second + 24U, second + 24U + 17U,
              [](std::byte value) { return value == std::byte{0xb2}; }) &&
              std::ranges::all_of(
                  second + 24U + 17U, both.end(),
                  [](std::byte value) { return value == std::byte{0x22}; }),
          "Raw overlay did not preserve the partial final sector");
}
...
void testDirectMode2Bin() {
  sf::test::TemporaryDirectory temporary{"sf_raw_sector_tests"};
  const auto bin_path = temporary.path() / "disc.BIN";
  writeTrack(bin_path);

  auto source = sf::disc::RawSectorSource::open(bin_path);
  require(source.sectorCount() == 3U,
          "Direct BIN image did not expose every physical sector");
  const auto first = source.readSector(0U);
  require(first.front() == std::byte{0x09} && first.back() == std::byte{0x09},
          "Direct BIN image unexpectedly applied a CUE index offset");
}
...
void testReadAheadAndRandomReadSemantics() {
  ...
  std::vector<std::byte> direct(70U *
                                sf::disc::RawSectorSource::raw_sector_size);
  source.readSectors(10U, direct);
  for (std::uint32_t index = 0U; index < 70U; ++index) {
    requirePatternedSector(std::span<const std::byte>{direct}.subspan(
                               static_cast<std::size_t>(index) *
                                   sf::disc::RawSectorSource::raw_sector_size,
                               sf::disc::RawSectorSource::raw_sector_size),
                           10U + index,
