...
bool texturePageMatchesVlf(std::span<const std::byte> vlf,
  ...
  return std::ranges::equal(vlfPage(vlf, page_mask, page), texture_bank_page);
}
...
void uploadTexturePageAt(unsigned int physical_page,
  ...
  if (physical_page >= psx_texture_page_count) {
    std::vector<std::uint16_t> words(texture_page_bytes /
                                     sizeof(std::uint16_t));
    for (std::size_t index = 0U; index < words.size(); ++index) {
      words[index] = readLe16(bytes, index * sizeof(std::uint16_t));
    }
    GR_UploadVRAMAliasPage(
        static_cast<int>(physical_page - psx_texture_page_count), words.data());
    return;
  }
...
void uploadTexturePageBlockAt(unsigned int physical_page,
  ...
  if (physical_page < psx_texture_page_count) {
    uploadTimBlockAt(block,
                     static_cast<std::uint16_t>(
                         (physical_page & 15U) * page_width + local_x),
                     static_cast<std::uint16_t>(
                         (physical_page > 15U ? page_height : 0U) + local_y));
    return;
  }

  std::vector<std::uint16_t> page(texture_page_bytes / sizeof(std::uint16_t));
