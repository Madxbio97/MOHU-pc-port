...
private:
  // 256 stereo frames are 5.8 ms at the native SPU rate. Eight queued blocks
  // cover more than two host frames without the excessive OpenAL churn of the
  // old 128-frame blocks. FIFO order remains exact.
  static constexpr std::size_t frames_per_buffer = 256U;
  static constexpr std::size_t maximum_queued_buffers = 24U;
  static constexpr std::size_t guest_frames_per_tick =
      (psx::Spu::sample_rate + 60U - 1U) / 60U;
  ...
  // Startup stays responsive; starvation recovery separately retains one
  // maximum atomic host-production batch behind the current callback request.
  std::size_t minimum_start_frames_{};
  std::atomic<std::size_t> callback_request_frames_{};
  std::atomic<std::size_t> callback_recovery_frames_{};
