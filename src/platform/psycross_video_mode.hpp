...
class ScopedPsyCrossAspectMode final {
public:
  explicit ScopedPsyCrossAspectMode(PresentationContent content) noexcept
      : restore_(g_cfg_aspectMode) {
    const auto configured = restore_ == PSYX_ASPECT_ADAPTIVE
                                ? AspectRatioMode::adaptive
                                : AspectRatioMode::original_4_3;
    g_cfg_aspectMode = presentationAspectRatio(configured, content) ==
                               AspectRatioMode::adaptive
                           ? PSYX_ASPECT_ADAPTIVE
                           : PSYX_ASPECT_ORIGINAL_4_3;
  }

  ~ScopedPsyCrossAspectMode() { g_cfg_aspectMode = restore_; }

  ScopedPsyCrossAspectMode(const ScopedPsyCrossAspectMode &) = delete;
  ScopedPsyCrossAspectMode &
  operator=(const ScopedPsyCrossAspectMode &) = delete;

private:
  int restore_{};
};

inline void configurePsyCrossVideoMode(PsyCrossVideoMode mode,
                                       bool reset_graph) noexcept {
...
public:
  ScopedPsyCrossVideoMode(PsyCrossVideoMode active,
                          PsyCrossVideoMode restore) noexcept
      : restore_(restore) {
    ...
    PsyX_EndScene();
    aspect_mode_.emplace(PresentationContent::authored_4_3);
...
private:
  ...
  std::optional<ScopedPsyCrossAspectMode> aspect_mode_;
};
