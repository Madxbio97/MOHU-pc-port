...
PsyCrossWindowMode::PsyCrossWindowMode(bool start_fullscreen) {
  if (g_window == nullptr) {
    PsyX_Log_Warning(
        "Window mode controller disabled: SDL window is unavailable\n");
    return;
  }

  SDL_SetWindowMinimumSize(g_window, minimum_window_width,
...
PsyCrossWindowMode::~PsyCrossWindowMode() {
  ...
  if (current_filter == &PsyCrossWindowMode::filterEvent &&
      current_userdata == this) {
...
void PsyCrossWindowMode::captureWindowedBounds() noexcept {
  ...
  SDL_GetWindowSize(g_window, &windowed_bounds_.width,
                    &windowed_bounds_.height);
  windowed_bounds_.valid =
      windowed_bounds_.width > 0 && windowed_bounds_.height > 0;
}

void PsyCrossWindowMode::observeWindowEvent(
