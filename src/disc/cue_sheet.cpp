...
CueSheet CueSheet::load(const std::filesystem::path& cue_path) {
    if (uppercase(cue_path.extension().string()) == ".BIN") {
        if (!std::filesystem::is_regular_file(cue_path)) {
            throw core::Error{core::ErrorCode::not_found,
                              "Raw BIN image was not found: " + cue_path.string()};
        }
        CueSheet result;
        result.path_ = cue_path;
        result.data_track_ = DataTrack{cue_path, TrackMode::mode2_2352, 0U};
        return result;
    }
