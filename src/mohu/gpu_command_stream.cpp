...
bool GpuCommandStream::appendGp0(std::uint32_t value,
  ...
  const auto source_addresses_size = frame_source_addresses_.size();
  ...
  try {
    ...
    frame_source_addresses_.push_back(source_address);
    ...
    while (frame_source_addresses_.size() > source_addresses_size) {
      frame_source_addresses_.pop_back();
    }
...
bool GpuCommandStream::writeGp0(std::uint32_t value) noexcept {
  return appendGp0(value, direct_source_address, nullptr, 0U);
}

bool GpuCommandStream::writeGp0Projected(
    std::uint32_t value, const sf::psx::GteProjectedVertex *projected,
    std::uint64_t source_identity) noexcept {
  return appendGp0(value, direct_source_address, projected, source_identity);
}
...
bool GpuCommandStream::writeGp0FromRam(
    ...
    const sf::psx::GteProjectedVertex *projected,
    std::uint64_t source_identity) noexcept {
  return appendGp0(value, source_address, projected, source_identity);
}
