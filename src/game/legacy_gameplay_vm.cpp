...
bool validGteProjectedVertices(const psx::GteState &gte) noexcept {
  for (std::size_t index{}; index < gte.projected.size(); ++index) {
    const auto &projected = gte.projected[index];
    if (!projected.valid) {
      continue;
    }
    const auto finite = std::isfinite(projected.view_x) &&
    ...
    if (!finite || projected.view_z <= 0.0F ||
        projected.packed_sxy != gte.data[12U + index]) {
      return false;
    }
  }
  return true;
...
bool validCpuSnapshot(const psx::R3000State &cpu) noexcept {
  ...
  return cpu.gpr[0] == 0U && (cpu.pc & instruction_alignment_mask) == 0U &&
         ...
         validGteProjectedVertices(cpu.gte);
}
...
bool LegacyGameplayVm::setRetailVibrationEnabled(bool enabled) noexcept {
  ...
  if (!enabled && (!runtime_.write8(retail_pad_motor_table, 0U) ||
