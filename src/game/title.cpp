...
std::uint8_t approachTitleBrightness(std::uint8_t current,
                                     std::uint8_t target) noexcept {
  if (target == 0) {
    return current <= TitleMenu::brightness_step
               ? 0
               : static_cast<std::uint8_t>(current -
                                           TitleMenu::brightness_step);
  }
  ...
  return std::ranges::all_of(slots, [mission_count](const auto &slot) {
    ...
    if (!slot.occupied) {
      return !slot.campaign_complete && !slot.pending_eol_mission &&
             !slot.carry && slot.difficulty == CampaignDifficulty::original;
    }
    if (slot.carry && !validCampaignCarry(*slot.carry)) {
      return false;
    }
    if (slot.pending_eol_mission) {
      return !slot.campaign_complete && slot.mission_index < mission_count &&
    ...
    if (slot.campaign_complete) {
      return mission_count != 0U && slot.mission_index == mission_count - 1U &&
             !slot.carry;
    }
    return slot.mission_index < mission_count &&
           (!slot.carry || (slot.mission_index != 0U &&
                            campaignMissionsShareCarry(slot.mission_index - 1U,
                                                       slot.mission_index)));
  });
}

std::filesystem::path saveSiblingPath(const std::filesystem::path &path,
...
TitleAssets::TitleAssets(std::vector<TitleSprite> sprites)
...
TitleAssets TitleAssets::load(GameDisc &disc) {
  ...
  auto archive =
      assets::HogArchive::parse(disc.image().readFile("COMMON/TITLE.HOG"));
  std::vector<TitleSprite> sprites;
  sprites.reserve(definitions.size());
  for (const auto &definition : definitions) {
    const auto localized =
        readLocalizedAsset(std::string{"title/"} + definition.name);
    const auto source = localized ? std::span<const std::byte>{*localized}
...
TitleMovies TitleMovies::load(GameDisc &disc) {
  ...
  constexpr std::array<const char *, movie_count> paths{
      "SOL/989LOGO.STR", "SOL/EIDETIC.STR",  "SOL/INTRO.STR",
      "SOL/TITLE.STR",   "SOL/TRAINING.STR",
  };
...
TitleCommand TitleMenu::update(const TitleInput &input,
  ...
  if (phase_ == TitlePhase::load_slots) {
    ...
    if (input.pointer_selection && *input.pointer_selection < row_count) {
      load_slot_selection_ = *input.pointer_selection;
    }
    if (input.next) {
  ...
  if (phase_ == TitlePhase::select_difficulty) {
    ...
    if (input.pointer_selection &&
        *input.pointer_selection < difficulty_count) {
      difficulty_selection_ = *input.pointer_selection;
    }
    if (input.next) {
      difficulty_selection_ = (difficulty_selection_ + 1U) % difficulty_count;
    }
    if (input.previous) {
      difficulty_selection_ = difficulty_selection_ == 0U
                                  ? difficulty_count - 1U
  ...
  if (input.pointer_selection && itemEnabled(*input.pointer_selection)) {
    selection_ = *input.pointer_selection;
  }
...
bool TitleMenu::itemEnabled(std::size_t item) const noexcept {
  return (phase_ == TitlePhase::searching || phase_ == TitlePhase::menu) &&
         item < item_count && !(item == 1U && phase_ == TitlePhase::searching);
}
...
void TitleMenu::updateVisuals(std::uint32_t background_movie_frame) noexcept {
  for (std::size_t index = 0; index < brightness_.size(); ++index) {
    const auto is_load =
        index == static_cast<std::size_t>(TitleVisual::load_game);
    const auto is_search =
        index == static_cast<std::size_t>(TitleVisual::searching);
    ...
    const auto target = hidden                ? 0U
                        : index == selection_ ? selected_brightness
                                              : idle_brightness;
    brightness_[index] = approachTitleBrightness(
        brightness_[index], static_cast<std::uint8_t>(target));
  }
  for (std::size_t index = 0; index < difficulty_brightness_.size(); ++index) {
    const auto target = phase_ != TitlePhase::select_difficulty ? 0U
...
std::string serializeTitleSaveSlots(const TitleSaveSlots &slots) {
  ...
  for (std::size_t index = 0; index < slots.size(); ++index) {
    ...
    output << index << ' ' << (slot.occupied ? 1 : 0) << ' '
           << slot.mission_index << ' ' << (slot.campaign_complete ? 1 : 0)
    ...
    if (slot.carry) {
      output << ' ' << static_cast<unsigned>(slot.carry->current_weapon) << ' '
             << slot.carry->owned_weapons << ' ' << slot.carry->health << ' '
             << slot.carry->armor;
...
std::optional<TitleSaveSlots> parseTitleSaveSlots(std::string_view bytes) {
  ...
  const auto version_three_or_newer = magic == "SFPC_SAVE_V3" ||
                                      magic == "SFPC_SAVE_V4" ||
                                      magic == "SFPC_SAVE_V5";
  ...
  for (std::size_t expected = 0; expected < slots.size(); ++expected) {
    ...
    if (!(input >> index >> occupied >> mission_index) ||
        ...
        (version_five && !(input >> difficulty)) || index != expected ||
        occupied > 1U || campaign_complete > 1U || pending_eol > 1U ||
        has_carry > 1U ||
        difficulty > static_cast<unsigned>(CampaignDifficulty::agent) ||
    ...
    if (has_carry != 0U) {
      ...
      if (!(input >> current_weapon >> state.owned_weapons >> state.health >>
            state.armor) ||
    ...
    slots[index] = TitleSaveSlot{
        occupied != 0U,
        mission_index,
        campaign_complete != 0U,
        pending_eol != 0U ? std::optional<std::uint32_t>{pending_eol_mission}
                          : std::nullopt,
        std::move(carry),
...
TitleSaveLoadResult
loadTitleSaveSlotsFile(const std::filesystem::path &path) noexcept {
  try {
...
bool storeTitleSaveSlotsFile(const std::filesystem::path &path,
  ...
  try {
    ...
    if (readSaveFile(backup).status != SaveFileReadStatus::loaded) {
      ...
      if (!error) {
        std::filesystem::copy_file(path, temporary_backup,
      ...
      if (error) {
        std::error_code ignored;
        static_cast<void>(std::filesystem::remove(temporary_backup, ignored));
...
TitleSaveLocation
titleSaveLocation(const std::filesystem::path &cue_path,
...
TitleSaveLocation
defaultTitleSaveLocation(const std::filesystem::path &cue_path,
                         std::string_view supported_game_serial) noexcept {
  try {
    return titleSaveLocation(cue_path, supported_game_serial,
                             platformUserDataRoot());
  } catch (...) {
    try {
      return titleSaveLocation(cue_path, supported_game_serial,
...
TitleSaveMigrationStatus
migrateLegacyTitleSaveSlotsFile(const TitleSaveLocation &location) noexcept {
  try {
