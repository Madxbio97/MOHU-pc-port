...
constexpr std::uint8_t command_get_tn = 0x13U;
constexpr std::uint8_t command_get_td = 0x14U;
...
bool CdRomController::prepareDmaRead(std::uint64_t word_count) noexcept {
  ...
  // so their data position is already nonzero before the payload DMA.
  ...
  return canReadDmaWords(word_count);
}
...
void CdRomController::writeCommand(std::uint8_t command) noexcept {
  ...
  if (command < state_.command_counts.size()) {
    ++state_.command_counts[command];
  }
...
void CdRomController::executeCommand() noexcept {
  ...
  case command_get_tn: {
    ...
    const std::array response{statusByte(), std::uint8_t{0x01U},
                              std::uint8_t{0x01U}};
    state_.command_phase = CdRomCommandPhase::idle;
    setResponse(interrupt_acknowledge, response);
    return;
  }
  ...
  case command_get_td: {
    ...
    std::uint32_t track{};
    if (!decodeBcd(state_.parameters[0], 100U, track) || track > 1U) {
      clearParameters();
      state_.command_phase = CdRomCommandPhase::idle;
      setErrorResponse(0x10U);
      return;
    ...
    std::uint64_t absolute_sector = absolute_lead_in_sectors;
    if (track == 0U) {
      absolute_sector += media_->sectorCount();
    }
    const auto minute =
        static_cast<std::uint32_t>(absolute_sector / (60U * 75U));
    const auto second =
        static_cast<std::uint32_t>((absolute_sector / 75U) % 60U);
    const std::array response{statusByte(), encodeBcd(minute),
                              encodeBcd(second)};
    clearParameters();
    state_.command_phase = CdRomCommandPhase::idle;
    setResponse(interrupt_acknowledge, response);
