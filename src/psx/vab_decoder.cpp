...
constexpr std::uint32_t preview_sample_base = 0x1000U;
constexpr std::uint16_t preview_sample_address =
    static_cast<std::uint16_t>(preview_sample_base >> 3U);
...
[[nodiscard]] bool uploadSample(Spu &spu,
  ...
  if (sample.empty() || (sample.size() & 15U) != 0U ||
      sample.size() > Spu::ram_size - preview_sample_base) {
    return false;
  }
  if (!spu.writeRegister(transfer_address, preview_sample_address) ||
...
VabDecodeResult decodeVabSound(std::span<const std::byte> header,
  ...
  const auto configured =
      ...
      spu.writeRegister(voice_start_address, preview_sample_address) &&
      spu.writeRegister(voice_repeat_address, preview_sample_address) &&
  ...
  // KON takes effect after one 44.1 kHz boundary. Discard that pre-key frame
  // before collecting the native preview waveform.
  spu.mixFrames(1U);
  std::array<SpuPcmFrame, 1U> pre_key_frame{};
  if (spu.takePcm(pre_key_frame) != 1U ||
      spu.state().voices[0].active == 0U) {
    result.status = VabDecodeStatus::invalid_sample;
    return result;
  }
