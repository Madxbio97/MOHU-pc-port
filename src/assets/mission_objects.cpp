...
std::uint16_t readLe16(std::span<const std::byte> bytes, std::size_t offset) {
  if (offset > bytes.size() || bytes.size() - offset < sizeof(std::uint16_t)) {
    throw core::Error{core::ErrorCode::invalid_format,
                      "Truncated mission-object integer"};
  }
...
std::string readString(std::span<const std::byte> bytes, std::size_t offset) {
  if (offset >= bytes.size()) {
    throw core::Error{core::ErrorCode::invalid_format,
                      "Mission-object string offset is invalid"};
  }
  std::string result;
  while (offset < bytes.size() && bytes[offset] != std::byte{0}) {
    ...
    if (character < 0x20U || character > 0x7eU) {
      throw core::Error{core::ErrorCode::invalid_format,
                        "Mission-object model name is not ASCII"};
    }
  ...
  if (offset == bytes.size()) {
    throw core::Error{core::ErrorCode::invalid_format,
                      "Mission-object model name is truncated"};
  }
  return result;
}

std::vector<MissionPathPoint> readPatrolPath(std::span<const std::byte> bytes,
  ...
  std::array<std::uint8_t, 256U> result_indices{};
  ...
  for (;;) {
    if (visited[node]) {
      ...
      loops = true;
      loop_start = result_indices[node];
      break;
    }
    visited[node] = true;
    if (result.size() >= result_indices.size()) {
      throw core::Error{core::ErrorCode::invalid_format,
                        "Mission patrol path has too many nodes"};
    }
    result_indices[node] = static_cast<std::uint8_t>(result.size());
    ...
    if (offset > bytes.size() || bytes.size() - offset < path_node_size ||
        std::to_integer<std::uint8_t>(bytes[offset + path_marker_offset]) !=
            path_marker) {
      throw core::Error{core::ErrorCode::invalid_format,
                        "Mission patrol path is invalid"};
    }
    ...
    const auto next =
        std::to_integer<std::uint8_t>(bytes[offset + path_next_offset]);
    if (next == path_end) {
      break;
    }
    node = next;
...
MissionObjects::MissionObjects(
    ...
    : objects_(std::move(objects)), definitions_(std::move(definitions)),
      room_objects_(std::move(room_objects)),
      object_rooms_(std::move(object_rooms)), player_index_(player_index) {}

MissionObjects MissionObjects::parse(std::span<const std::byte> bytes) {
  if (bytes.size() < header_size) {
    throw core::Error{core::ErrorCode::invalid_format,
                      "Mission-object header is truncated"};
  }
  const auto room_count =
      static_cast<std::size_t>(readLe32(bytes, room_count_offset));
  const auto count =
      static_cast<std::size_t>(readLe32(bytes, object_count_offset));
  const auto definition_count =
      static_cast<std::size_t>(readLe32(bytes, definition_count_offset));
  const auto definitions_offset =
      static_cast<std::size_t>(readLe32(bytes, definition_table_offset));
  const auto table_offset =
      static_cast<std::size_t>(readLe32(bytes, object_table_offset));
  const auto rooms_offset =
      static_cast<std::size_t>(readLe32(bytes, room_table_offset));
  const auto player_index =
      static_cast<std::size_t>(readLe32(bytes, player_index_offset));
  if (room_count == 0 ||
      room_count > std::numeric_limits<std::uint16_t>::max() || count == 0 ||
      definition_count == 0 || player_index >= count ||
      definitions_offset > bytes.size() ||
      definition_count >
          (bytes.size() - definitions_offset) / definition_size ||
      table_offset > bytes.size() || rooms_offset > bytes.size() ||
      room_count >
          (bytes.size() - rooms_offset) / (2U * sizeof(std::uint32_t)) ||
      count > (bytes.size() - table_offset) / object_size) {
    throw core::Error{core::ErrorCode::invalid_format,
                      "Mission-object table is invalid"};
  }
  ...
  for (std::size_t index = 0; index < count; ++index) {
    ...
    if (object.type >= definition_count) {
      throw core::Error{core::ErrorCode::invalid_format,
                        "Mission object has an invalid definition"};
    }
    for (std::size_t component = 0;
         component < object.transform.rotation.size(); ++component) {
      object.transform.rotation[component] = static_cast<std::int16_t>(
          readLe16(bytes, offset + rotation_offset + component * 2U));
    }
    object.transform.x =
        static_cast<std::int32_t>(readLe32(bytes, offset + position_offset));
    object.transform.y = static_cast<std::int32_t>(
        readLe32(bytes, offset + position_offset + 4U));
    object.transform.z = static_cast<std::int32_t>(
        readLe32(bytes, offset + position_offset + 8U));
    ...
    if (class_id == 0x01U || class_id == 0x35U || class_id == 0x73U) {
      object.patrol_path =
          readPatrolPath(bytes, object.path_data_offset,
                         object.patrol_path_loops, object.patrol_loop_start);
    }
    object.maximum_health = static_cast<std::int16_t>(
        readLe16(bytes, offset + maximum_health_offset));
    object.health =
        static_cast<std::int16_t>(readLe16(bytes, offset + health_offset));
  ...
  for (std::size_t room = 0; room < room_count; ++room) {
    ...
    const auto room_object_count =
        static_cast<std::size_t>(readLe32(bytes, room_offset));
    const auto indices_offset =
        static_cast<std::size_t>(readLe32(bytes, room_offset + 4U));
    if (indices_offset > bytes.size() ||
        room_object_count >
            (bytes.size() - indices_offset) / sizeof(std::uint32_t)) {
      throw core::Error{core::ErrorCode::invalid_format,
                        "Mission room-object list is invalid"};
    }
    std::vector<std::uint16_t> indices;
    indices.reserve(room_object_count);
    for (std::size_t index = 0; index < room_object_count; ++index) {
      const auto object_index =
          readLe32(bytes, indices_offset + index * sizeof(std::uint32_t));
      if (object_index >= count ||
          object_index > std::numeric_limits<std::uint16_t>::max()) {
        throw core::Error{core::ErrorCode::invalid_format,
                          "Mission room has an invalid object"};
      }
      const auto checked_object = static_cast<std::uint16_t>(object_index);
      indices.push_back(checked_object);
      object_rooms[checked_object].push_back(static_cast<std::uint16_t>(room));
    }
    room_objects.push_back(std::move(indices));
  }
  return MissionObjects{std::move(objects), std::move(definitions),
                        std::move(room_objects), std::move(object_rooms),
                        player_index};
}

const MissionObjectDefinition &
MissionObjects::definition(std::size_t index) const {
  if (index >= definitions_.size()) {
    throw core::Error{core::ErrorCode::invalid_argument,
                      "Mission-object definition is invalid"};
  }
  return definitions_[index];
}

std::span<const std::uint16_t>
MissionObjects::objectsInRoom(std::size_t room) const {
  if (room >= room_objects_.size()) {
    throw core::Error{core::ErrorCode::invalid_argument,
                      "Mission room is invalid"};
  }
