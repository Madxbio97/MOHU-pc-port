...
public:
  ...
  [[nodiscard]] const std::vector<TitleSprite> &sprites() const noexcept {
    return sprites_;
  }
...
public:
  ...
  [[nodiscard]] std::vector<TitleMovie> &sequence() noexcept {
    return sequence_;
  }
  [[nodiscard]] const std::vector<TitleMovie> &sequence() const noexcept {
    return sequence_;
  }
...
[[nodiscard]] std::optional<TitleSaveSlots>
parseTitleSaveSlots(std::string_view bytes);
...
[[nodiscard]] TitleSaveLoadResult
loadTitleSaveSlotsFile(const std::filesystem::path &path) noexcept;
[[nodiscard]] bool
storeTitleSaveSlotsFile(const std::filesystem::path &path,
                        const TitleSaveSlots &slots) noexcept;
[[nodiscard]] TitleSaveLocation
...
[[nodiscard]] TitleSaveMigrationStatus
...
public:
  ...
  static constexpr std::size_t visual_count =
      static_cast<std::size_t>(TitleVisual::count);
  ...
  [[nodiscard]] TitleCommand update(const TitleInput &input,
  ...
  [[nodiscard]] const TitleSaveSlots &saveSlots() const noexcept {
    return save_slots_;
  }
  [[nodiscard]] std::size_t loadSlotSelection() const noexcept {
    return load_slot_selection_;
  }
  ...
  difficultyBrightness(std::size_t index) const noexcept {
    return index < difficulty_brightness_.size() ? difficulty_brightness_[index]
                                                 : 0U;
  }
