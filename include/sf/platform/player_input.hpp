...
struct KeyboardMouseActionMetadata {
  KeyboardMouseAction action{KeyboardMouseAction::move_forward};
  std::string_view name;
  std::string_view config_key;
  KeyboardMouseInput default_input{KeyboardMouseInput::none};
};

[[nodiscard]] KeyboardMouseBindings defaultKeyboardMouseBindings() noexcept;
[[nodiscard]] const std::array<KeyboardMouseActionMetadata,
...
// right; positive pitch is up. W/S and A/D own locomotion, mouse/right-stick
// own view, arrows provide keyboard turning, and Q/E retain corner peek.
struct PlayerInput {
  double move_forward{};
  double move_strafe{};
  double move_peek{};
